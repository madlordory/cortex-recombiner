...
const utilsCreator = require('./utils');
const defaultOpts = {
    base: __dirname,
    source_path: './neurons',
...
const TASK = {
    init: function(opts) {
        return function*() {
            ...
            OPTIONS.target_path = path.isAbsolute(OPTIONS.target_path) && OPTIONS.target_path || path.join(OPTIONS.base, OPTIONS.target_path, OPTIONS.scope);

            utils = utilsCreator(OPTIONS);
            /*
            ...
            if (!fs.existsSync(cortex_json_file)) {
                console.warn('It seems that your project did not include any cortex package');
                return;
            }
             ...
             * */
            const files = fs.existsSync(OPTIONS.source_path) && fs.readdirSync(OPTIONS.source_path) || [];
            if (files && files.length === 0) {
                console.warn('dir neurons is empty, try run : cortex install --stable-only');
                return;
            }
    ...
    process_dependencies: function*() {
        if (fs.existsSync(OPTIONS.target_path)) {
            const dirs = fs.readdirSync(OPTIONS.target_path);
            /*
            ...
            yield dirs && _.map(dirs, function(pkg) {
                return function*() {
                    yield utils.processDependencies(path.join(OPTIONS.target_path, pkg), CORTEX_PACKAGES);
                }
            }) || [];
        }
    }
