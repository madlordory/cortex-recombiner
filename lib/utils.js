...
module.exports = function(OPTIONS) {
    const utils = {
        ...
        transformCortexPkg: function(source_path, pkg_name, ver, dest) {
            return function*() {
                const src_path = path.join(source_path, pkg_name, ver);//选择本地最新的cortex包版本
                const dst_path = path.join(dest, pkg_name);//设置node_modules/@cortex/文件夹下对于的包路径
                ...
                yield dirs && _.map(dirs, function(item) {
                    return function*() {
                        ...
                        if (fs.statSync(dst_path_item).isFile() && item === 'cortex.json') {
                            ...
                            const obj = fsep.readJsonSync(dst_path_item);
                            if (obj.hasOwnProperty('dependencies')) {
                                let dep = {};
                                let keys = _.keys(obj.dependencies);
                                _.map(keys, function(key) {
                                    dep[OPTIONS.scope + '/' + key] = obj.dependencies[key];
                                });
                ...
                const cortexConf = fsep.readJsonSync(path.join(src_path, 'cortex.json'));

                if (cortexConf && cortexConf.entries) {
                    yield cortexConf.entries && _.map(cortexConf.entries, function(entry) {
                        return function *() {
                            if (fs.existsSync(path.join(src_path, entry))) {
                                fs.existsSync(path.join(dst_path, entry)) && fs.unlinkSync(path.join(dst_path, entry));//删除js文件
                                yield utils.processJS(path.join(src_path, entry), dst_path, pkg_name);
                            }
                        }
                    }) || [];
                }
        ...
        processDependencies: function(pkg_dir_path, CORTEX_PACKAGES) {
            const scope = OPTIONS.scope;
            return function*() {
                const pkg_json_path = path.join(pkg_dir_path, 'package.json');
                if (fs.existsSync(pkg_json_path)) {
                    const json = fsep.readJsonSync(pkg_json_path);
                    try {
                        yield json && json.dependencies && _.map(json.dependencies, function(sem_ver, pkg) {
                            return function*() {
                                if (pkg.indexOf(scope + '/') !== -1) {
                                    pkg = pkg.substring(8);
                                }
                                const prefer_ver = utils.chooseCorrectVersion(sem_ver, CORTEX_PACKAGES[pkg].versions, OPTIONS.noBeta);
                                const currentTopLevelVer = CORTEX_PACKAGES[pkg].top;
                                /*
                                ...
                                if (prefer_ver !== currentTopLevelVer) {
                                    let cortex_path = path.join(pkg_dir_path, 'node_modules', scope);
                                    yield fsep.ensureDirAsync(cortex_path);
                                    const dest_path = yield utils.transformCortexPkg(OPTIONS.source_path, pkg, prefer_ver, cortex_path);
                                    yield utils.processDependencies(dest_path, CORTEX_PACKAGES);
                                }
                            }
            ...
            return function*() {
                if (fs.existsSync(src)) {//读取删除js对应的源js文件
                    ...
                    yield result && _.map(result, function(obj) {
                        return function*() {
                            const pkg_name = obj.key;
                            let code = obj.code;
                            const ss = code.substring(code.indexOf('{') + 1).split('}');
                            ss.pop();
                            code = ss.join('}');
                            // console.log(code);
                            const _t = pkg_name.split('@');
                            const module_name = _t[0];
                            const module_path = (_t[1].indexOf('/') !== -1 ? _t[1].substring(_t[1].indexOf('/')) : '');
